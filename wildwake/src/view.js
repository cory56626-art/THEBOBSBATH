...
export class View{
 ...
 buildTerrain(){
  ...
  g.setAttribute('color',new T.Float32BufferAttribute(colors,3));this.terrain=new T.Mesh(g,new T.MeshStandardMaterial({vertexColors:true,roughness:1}));this.terrain.receiveShadow=true;this.terrain.userData.cpuTerrain=true;let coarse=terrainData(48),cg=new T.BufferGeometry();cg.setAttribute('position',new T.BufferAttribute(coarse.positions,3));cg.setIndex(new T.BufferAttribute(coarse.indices,1));cg.computeVertexNormals();this.terrain.userData.cpuGeometry=cg;this.root.add(this.terrain);
  ...
  for(let i=0;i<count;i++){let x=(r()-.5)*110,z=(r()-.5)*110,y=height(x,z);dummy.position.set(x,y-.02,z);let sc=y<WATER+.12?0:.7+r()*.9;dummy.scale.set(sc,sc,sc);dummy.rotation.y=r()*6.28;dummy.updateMatrix();this.grass.setMatrixAt(i,dummy.matrix);}this.grass.userData.cpuSkip=true;this.root.add(this.grass);
  // Ferns are flexible foliage; all trunks and stones have solid physics shapes.
  let fernGeo=new T.ConeGeometry(.43,.35,5),ferns=new T.InstancedMesh(fernGeo,leaf,330);for(let i=0;i<330;i++){let x=(r()-.5)*103,z=(r()-.5)*103;dummy.position.set(x,height(x,z)+.15,z);dummy.scale.set(1,height(x,z)<WATER?0:1,1);dummy.rotation.set(0,r()*6,0);dummy.updateMatrix();ferns.setMatrixAt(i,dummy.matrix);}ferns.userData.cpuStride=3;this.root.add(ferns);
