...
export function itemModel(id,n=1){let g=new T.Group();
 if(id==='spear'||id==='pike'){rod(g,[0,-.86,0],[0,.72,0],.035,wood);mesh(new T.ConeGeometry(.075,.27,4),bone,[0,.85,0],[1,1,1],g);for(let y of[.59,.63,.67])mesh(new T.TorusGeometry(.04,.012,4,8),fiber,[0,y,0],[1,1,1],g).rotation.x=Math.PI/2;}
 else if(id==='axe'||id==='pickaxe'){rod(g,[0,-.37,0],[0,.37,0],.045,wood);ell(g,[.1,.27,0],[id==='pickaxe'?.42:.25,id==='pickaxe'?.07:.15,.065],bone);for(let y of[.21,.27,.33])mesh(new T.TorusGeometry(.06,.018,4,8),fiber,[0,y,0],[1,1,1],g).rotation.x=Math.PI/2;}
 else if(id==='branch'){rod(g,[0,0,-.43],[0,0,.43],.047,wood);rod(g,[0,0,.14],[.16,.025,.3],.022,wood);}
 else if(id==='wood'||id==='plank'||id==='bridge')box(g,[0,0,0],id==='wood'?[.7,.24,.3]:id==='bridge'?[4,.24,1.2]:[2,.24,.3],wood);
 else if(id==='torch'){rod(g,[0,-.4,0],[0,.28,0],.045,wood);ell(g,[0,.31,0],[.09,.15,.09],fiber);mesh(new T.ConeGeometry(.09,.25,6),new T.MeshBasicMaterial({color:0xffca72}),[0,.49,0],[1,1,1],g);}
 else if(id==='waterskin'){ell(g,[0,0,0],[.17,.23,.08],mat(0x997957));rod(g,[0,.2,0],[0,.3,0],.04,wood);}
 else if(id==='rope'){mesh(new T.TorusGeometry(.13,.03,6,16),fiber,[0,0,0],[1,1,1],g);}
 else if(id==='bandage'){box(g,[0,0,0],[.21,.06,.15],mat(0xdddac5));}
...
function sign(text){let c=document.createElement('canvas');c.width=512;c.height=128;let ctx=c.getContext('2d');ctx.fillStyle='#283e32';ctx.fillRect(0,0,512,128);ctx.strokeStyle='#9dae8b';ctx.lineWidth=3;ctx.strokeRect(8,8,496,112);ctx.fillStyle='#e3dec1';ctx.textAlign='center';ctx.font='500 31px sans-serif';ctx.fillText(text,256,76);let texture=new T.CanvasTexture(c);texture.colorSpace=T.SRGBColorSpace;let material=new T.MeshBasicMaterial({map:texture});material.userData.label=text;return material;}
export class View{
 constructor(canvas,sim,settings){
  this.sim=sim;this.canvas=canvas;let context=canvas.getContext('webgl2',{antialias:true,alpha:false});this.renderer=context?new T.WebGLRenderer({canvas,context,antialias:true}):new SoftwareRenderer(canvas);this.renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));this.renderer.shadowMap.enabled=true;this.renderer.shadowMap.type=T.PCFSoftShadowMap;this.renderer.outputColorSpace=T.SRGBColorSpace;this.renderer.toneMapping=T.ACESFilmicToneMapping;this.renderer.toneMappingExposure=1.15;
  this.scene=new T.Scene();this.scene.background=new T.Color(0xa8c6b9);this.scene.fog=new T.FogExp2(0xa8c6b9,.014);
  this.camera=new T.PerspectiveCamera(70,innerWidth/innerHeight,.05,350);this.scene.add(this.camera);this.camera.rotation.order='YXZ';
  ...
  this.held=new T.Group();this.camera.add(this.held);this.heldItem=null;this.torchLight=new T.PointLight(0xffc078,0,18,1.5);this.torchLight.position.set(.3,0,-.7);this.camera.add(this.torchLight);this.trajectory=new T.Group();for(let i=0;i<28;i++){let dot=mesh(new T.SphereGeometry(.045,6,4),new T.MeshBasicMaterial({color:0xe4e9b0}),[0,0,0],[1,1,1],this.trajectory);dot.castShadow=false;}this.scene.add(this.trajectory);this.trajectory.visible=false;
 ...
 buildTerrain(){
  ...
  g.setAttribute('color',new T.Float32BufferAttribute(colors,3));this.terrain=new T.Mesh(g,new T.MeshStandardMaterial({vertexColors:true,roughness:1}));this.terrain.receiveShadow=true;this.terrain.userData.cpuTerrain=true;let coarse=terrainData(144),cg=new T.BufferGeometry();cg.setAttribute('position',new T.BufferAttribute(coarse.positions,3));cg.setIndex(new T.BufferAttribute(coarse.indices,1));cg.computeVertexNormals();this.terrain.userData.cpuGeometry=cg;this.root.add(this.terrain);
  ...
  let r=rng(442),count=5400,grass=new T.BufferGeometry(),gp=[-.035,0,0,.035,0,0,.012,.37,.02,-.025,0,-.03,.03,0,.03,-.1,.24,.04];grass.setAttribute('position',new T.Float32BufferAttribute(gp,3));grass.computeVertexNormals();this.grass=new T.InstancedMesh(grass,new T.MeshStandardMaterial({color:0x8b9a60,side:T.DoubleSide,roughness:1}),count);let dummy=new T.Object3D();
  for(let i=0;i<count;i++){let span=i<2700?110:366,x=(r()-.5)*span,z=(r()-.5)*span,y=height(x,z);dummy.position.set(x,y-.02,z);let sc=y<WATER+.12?0:.7+r()*.9;dummy.scale.set(sc,sc,sc);dummy.rotation.y=r()*6.28;dummy.updateMatrix();this.grass.setMatrixAt(i,dummy.matrix);}this.grass.userData.cpuSkip=true;this.root.add(this.grass);
  // Ferns are flexible foliage; all trunks and stones have solid physics shapes.
  let fernGeo=new T.ConeGeometry(.43,.35,5),ferns=new T.InstancedMesh(fernGeo,leaf,700);for(let i=0;i<700;i++){let span=i<330?103:365,x=(r()-.5)*span,z=(r()-.5)*span;dummy.position.set(x,height(x,z)+.15,z);dummy.scale.set(1,height(x,z)<WATER?0:1,1);dummy.rotation.set(0,r()*6,0);dummy.updateMatrix();ferns.setMatrixAt(i,dummy.matrix);}ferns.userData.cpuStride=3;this.root.add(ferns);
 ...
 buildForest(){
  ...
  this.crowns.userData.cpuStride=2;this.trunks.castShadow=true;this.trunks.receiveShadow=true;this.crowns.castShadow=true;this.crowns.receiveShadow=true;
  trees.forEach((t,i)=>{let y=height(t.x,t.z),s=t.scale;dummy.position.set(t.x,y+3*s,t.z);dummy.rotation.set(0,t.seed*6.28,0);dummy.scale.set(s,s,s);dummy.updateMatrix();this.trunks.setMatrixAt(i,dummy.matrix);this.treeMeshIndices.set(t.id,i);
   ...
   for(let j=0;j<2;j++){let a=V(t.x,y+4*s,t.z),b=V(t.x+Math.cos(t.seed*6+j*Math.PI)*1.85*s,y+4*s,t.z+Math.sin(t.seed*6+j*Math.PI)*1.85*s);dummy.position.copy(a).lerp(b,.5);dummy.quaternion.setFromUnitVectors(V(0,1,0),b.clone().sub(a).normalize());dummy.scale.set(.18*s,a.distanceTo(b),.18*s);dummy.updateMatrix();this.limbs.setMatrixAt(i*2+j,dummy.matrix);}
 ...
 buildLandmarks(){
  ...
  let r=rng(728);for(let i=0;i<34;i++){let a=i/34*Math.PI*2,rad=239+r()*10;ell(this.root,[Math.cos(a)*rad,7+ r()*8,Math.sin(a)*rad],[9+r()*8,17+r()*15,12+r()*9],mat(i%2?0x788c7f:0x627b72));}
  for(let l of LANDMARKS.slice(4)){let y=height(l.x,l.z);let marker=new T.Group();marker.position.set(l.x,y,l.z);rod(marker,[0,0,0],[0,2.1,0],.08,wood);mesh(new T.PlaneGeometry(3,.75),sign(l.name.toUpperCase()),[0,1.8,.08],[1,1,1],marker);this.root.add(marker);}
 ...
 structureModel(s,preview=false){let g=new T.Group(),m=preview?this.ghostMat:wood,thatch=preview?m:mat(0x94a168);g.position.copy(s.p);g.rotation.y=s.yaw;
  ...
  else if(s.kind==='workbench'){box(g,[0,1.05,0],[2,.24,1.1],m);for(let x of[-.8,.8])for(let z of[-.4,.4])box(g,[x,.5,z],[.18,1,.18],m);ell(g,[.4,1.24,0],[.2,.1,.15],preview?m:rock);}
  else if(s.kind==='palisade'){for(let x=-1.5;x<=1.5;x+=.3){box(g,[x,1.25,0],[.28,2.5,.3],m);mesh(new T.ConeGeometry(.2,.25,4),m,[x,2.6,0],[1,1,1],g);}for(let y of[.5,1.8])box(g,[0,y,.18],[3.2,.13,.13],m);}
  else if(s.kind==='foundation'){box(g,[0,.35,0],[4,.7,4],m);for(let z=-1.8;z<=1.8;z+=.4)box(g,[0,.706,z],[3.98,.014,.02],preview?m:bark);}
  else if(s.kind==='bedroll'){box(g,[0,.08,0],[1.2,.16,2],thatch);ell(g,[0,.2,-.7],[.55,.2,.27],thatch);}
  else if(s.kind==='collector'){mesh(new T.CylinderGeometry(.65,.48,.9,10),m,[0,.45,0],[1,1,1],g);mesh(new T.CylinderGeometry(.57,.57,.03,16),preview?m:mat(0x81b4aa),[0,.88,0],[1,1,1],g);}
  else box(g,[0,.4,0],s.kind==='bridge'?[4,.24,1.2]:[2,.24,.3],m);return g;
 }
 ...
 setHeld(id){if(id===this.heldItem)return;this.held.clear();this.heldItem=id;if(!['spear','pike','axe','pickaxe','torch','waterskin','bandage','branch','stone','berries','meal','mushroom'].includes(id))return;let obj=itemModel(id);obj.scale.setScalar(id==='spear'||id==='pike'?.76:1);obj.rotation.set(-.35,0,-.35);this.held.add(obj);let hand=ell(this.held,[0,-.18,0],[.075,.11,.06],mat(0xb6a083));hand.castShadow=false;this.held.traverse(o=>{o.frustumCulled=false;o.castShadow=false;});}
 ...
 update(dt,{playing,yaw,pitch,charge=0,swing=0,menuTime=0}){
  ...
  if(sim.rain)sunlight*=.75;let day=new T.Color(0xb3cabe),night=new T.Color(0x233e44);this.scene.background.copy(night).lerp(day,sunlight);this.scene.fog.color.copy(this.scene.background);this.scene.fog.density=sim.rain?.015:.006;
  ...
  if(playing){let p=sim.position,v=sim.player.linvel(),speed=Math.hypot(v.x,v.z),bob=Math.sin(sim.elapsed*9)*.018*Math.min(1,speed/3);this.camera.position.set(p.x,p.y+(sim.input.crouch?.34:.76)+bob,p.z);this.camera.rotation.set(pitch,yaw,0);this.sun.position.set(p.x-24,p.y+45,p.z+24);this.sun.target.position.set(p.x,p.y,p.z);this.setHeld(sim.inv.slots[sim.selected]?.id);this.held.visible=true;this.held.position.set(.39,-.43-charge*.05-Math.sin(swing*Math.PI)*.17,-.62+charge*.23-Math.sin(swing*Math.PI)*.65);this.held.rotation.set(charge*.7-Math.sin(swing*Math.PI)*1.3,0,-charge*.12);}
  ...
  this.torchLight.intensity=playing&&sim.torchOn&&sim.inv.slots[sim.selected]?.id==='torch'?7:0;
  this.trajectory.visible=playing&&charge>.16;if(this.trajectory.visible){let sol=sim.throwSolution(this.camera.position,this.camera.getWorldDirection(V()),charge,this.aimAssist!==false);for(let [i,dot]of this.trajectory.children.entries()){let t=(i+1)*.035;dot.position.copy(sol.origin).addScaledVector(sol.velocity,t);dot.position.y-=9*t*t;dot.visible=dot.position.y>height(dot.position.x,dot.position.z)+.08;}}
